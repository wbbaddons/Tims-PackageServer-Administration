...
final class PackageServerUtil {
	...
	public static function generateAuthFile() {
		...
		$file->write("\t\"users\": {\n");
		$sql = "SELECT	userID, packageIdentifier, permissionString
		...
		while ($row = $userPackagesStatement->fetchArray()) {
			if (!isset($userPackages[$row['userID']])) $userPackages[$row['userID']] = array();
			$userPackages[$row['userID']][$row['packageIdentifier']] = $row['permissionString'];
		}
		$sql = "SELECT		user.userID, MIN(user.username) AS username, MIN(user.password) AS password, MIN(user.banned) AS banned, GROUP_CONCAT(user_to_group.groupID SEPARATOR ',') AS groupIDs
		...
		while ($row = $userStatement->fetchArray()) {
			...
			$userData = array(
				...
				'groups' => $groups,
				'packages' => isset($userPackages[$row['userID']]) ? $userPackages[$row['userID']] : array()
			);
		...
		$file->write("\t\"groups\": {\n");
		$sql = "SELECT	groupID, packageIdentifier, permissionString
		...
		while ($row = $groupPackagesStatement->fetchArray()) {
			if (!isset($groupPackages[$row['groupID']])) $groupPackages[$row['groupID']] = array();
		...
		$sql = "SELECT	packageIdentifier, permissionString
	...
	public static function getPackageServerPath() {
		if (!PACKAGESERVER_DIR) return false;
